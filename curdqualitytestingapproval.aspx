<%@ Page Title="" Language="C#" MasterPageFile="~/Operations.master" AutoEventWireup="true" CodeFile="curdqualitytestingapproval.aspx.cs" Inherits="curdqualitytestingapproval" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    <script type="text/javascript">
        ...
        function update_curd_packetwise_qualitytesting_click() {
        ...
        function fillqualitydetails(msg) {
            ...
            results += '<thead><tr><th scope="col">Product Name</th><th scope="col">Packet Size</th><th scope="col">Sample No</th><th scope="col">Used By Date</th><th scope="col">MRP</th><th scope="col">Temp</th><th scope="col">OT</th><th scope="col">Acidity</th><th scope="col">Structure</th></tr></thead></tbody>';
            for (var i = 0; i < msg.length; i++) {
                ...
                results += '<td scope="row" class="5" style="text-align:center;">' + msg[i].pot + '</td>';
                results += '<td scope="row" class="5" style="text-align:center;">' + msg[i].pacidity + '</td>';
                results += '<td scope="row" class="5" style="text-align:center;">' + msg[i].pstructure + '</td>';
                results += '<td style="display:none" class="7">' + msg[i].productid + '</td>';
                results += '<td style="display:none" class="8">' + msg[i].sno + '</td>';
                results += '<td><input id="btn_poplate" type="button"  onclick="getme(this)" name="submit" class="btn btn-success" value="Approve" /></td></tr>';
            }
            results += '</table></div>';
        ...
        function getme(thisid) {
            ...
            var confi = confirm("Do you want to Approve Transaction ?");
            if (confi) {
                var data = { 'op': 'Approve_curd_packetwise_qualitytesting_click', 'sno': sno, 'productid': productid
                };
                var s = function (msg) {
                    if (msg) {
                        alert(msg);
