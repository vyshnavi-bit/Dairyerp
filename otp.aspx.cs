        ...
        try
        {
            ...
            using (con = db.GetConnection())
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        Lbl_Approvaltype.Text = dr["val4"].ToString();
                    }
                }
            }
        }
            ...
            using (con = db.GetConnection())
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        //string strUrl = "http://103.16.101.52:8080/sendsms/bulksms?username=kapd-vyshnavi&password=vysavi&type=0&dlr=1&destination=" + no + "&source=VYSHRM&message=%20" + message1 + "";
